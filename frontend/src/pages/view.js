            ...
            <table className="border border-cyan-500 text-left m-auto ">
                ...
                <tr key ={Arsip.id}>
                    ...
                    <th className="border border-cyan-500 text-left m-auto p-1">
                        <button className="p-1 text-red-500" onClick={() => {if(window.confirm('Delete the item?')){deleteArsip(Arsip.id)};}}>HAPUS</button>
                        <a className="p-1 text-blue-600" href={`/edit/${Arsip.id}`}>EDIT</a>
                        <a className="p-1" href={`http://localhost:3001/uploads/${Arsip.file_pdf}`} download>DOWNLOAD</a>
                    </th>
                </tr>
                ))}
            </table>
